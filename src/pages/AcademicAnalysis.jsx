...
const AcademicAnalysis = () => {
  ...
  const handleContentSubmit = async e => {
    ...
    try {
      const analysisResults = await aiService.analyzeAcademic(content);
      ...
      setError(
        "Error al analizar el contenido académico. Por favor, intenta de nuevo."
      );
    } finally {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600">
            Detecta contenido académico generado por IA y verifica la
            originalidad
          </p>
        </div>
          ...
          <form onSubmit={handleContentSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Ingresa el contenido académico a analizar
              </label>
              <textarea
                value={content}
                onChange={e => setContent(e.target.value)}
                placeholder="Pega aquí el texto académico que quieres analizar (ensayos, tesis, artículos, etc.)..."
                className="w-full h-64 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
            ...
            >
              {isAnalyzing ? "Analizando..." : "Analizar Contenido Académico"}
            </button>
          </form>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="ml-2 font-medium">
                    {results.contentType || "No detectado"}
                  </span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="ml-2 font-medium">
                    {results.fullContent.split(" ").length}
                  </span>
                </div>
                <div>
                  <span className="text-gray-600">Originalidad:</span>
                  <span className="ml-2 font-medium">
                    {results.originality || "N/A"}%
                  </span>
                </div>
              </div>
            </div>
              ...
              >
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    ...
                    >
                      {results.verdict === "AI-Generated"
                        ? "Este contenido académico probablemente fue generado por IA"
                        : "Este contenido académico probablemente fue escrito por un humano"}
                    </p>
                  </div>
                </div>
              </div>
